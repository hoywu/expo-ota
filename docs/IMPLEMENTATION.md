...
expo-embedded-update-id: <uuid>    -- 可选, MVP 保留字段；当前不实现 rollBackToEmbedded
...
8. 构造 manifest JSON
9. 若请求带 `expo-expect-signature`:
     - 查该 App enabled 且未 disabled 的 signing key；不存在或不可用则返回 5xx，不降级为 unsigned
     - 签名 JSON bytes (RSA-SHA256 PKCS#1 v1.5)
     - 写 expo-signature header
10. app 已解析后的结果写一行 manifest_requests (occurred_at, result, served_update_id)；协议头错误或 app 不存在等解析前失败不入库
11. 返回响应
```
...
2. 若请求没有 `expo-expect-signature`：不签名，直接返回
3. 若请求带 `expo-expect-signature`，按 `app_id` 查 enabled 且未 disabled 的 signing key:
...
所有 `/api/admin/*` 端点需 `Authorization: Bearer <credential>`，凭据分两类：

- **管理员 JWT**（从 `POST /api/admin/login` 获取，HS256，24h 过期）：除 §6.4 上传端点外的全部管理操作
- **API Token**（从 dashboard 创建，`token_hash` 存 DB；只允许访问 §6.4 上传端点，见 §6.5）

§6.4 的 plan / finalize **不走 JWT 中间件**，由 `ApiTokenAuthMiddleware` 强制校验 `ota_pat_…` token。
...
仅 CLI/CI 使用；鉴权见 §6.5（API Token，非 JWT）。
...
在 `admin.api` 中单独注册为 Service Block 3（无 `jwt: Auth`），与 Block 2 的管理端点隔离。

### 6.5 API Token

API Token 是给 CLI/CI 上传使用的 App 级长期凭据，不等价于管理员 JWT。当前只支持 `publish` scope（命名沿用，语义为「允许 plan/finalize 上传链路」）：
...
- token **不能**访问 publish、App/User/Signing Key/Audit/Token 管理、rollback、cleanup 等操作；这些必须使用管理员 JWT。
- `ApiTokenAuthMiddleware` 在校验通过后把 `userId`（token 创建者）写入 request context（供 audit），**不**桥接 JWT。
...
| GET    | `/api/admin/apps/{appSlug}/signing-key`                     | 最新一条详情（含公钥；只读）            |
...
**用途**：管理员在 dashboard 点击「发布」后，手动将一个已 finalize 的 `pending` update 切到 `published`。**仅接受管理员 JWT，不接受 API Token。**
...
`manifest_uuid` 在 finalize / rollback 创建 update 行时**一次性计算并持久化**（不再每次请求重算）。finalize 为了支持无状态重试幂等，会从 canonical 输入中排除 `createdAt` 和 `id`；rollback/republish previous 会重新生成 `createdAt` 并基于新 manifest 计算 UUID。

```text
sha256(canonicalManifestJSONWithoutIdAndFinalizeCreatedAt)
  -> take first 16 bytes
  -> format as UUID string layout (8-4-4-4-12)
```

`canonicalManifestJSON` = 对 manifest 字段按固定 key 序序列化。当前 Go 实现使用 `encoding/json` 对 map key 的稳定排序来得到 canonical bytes。计算出 UUID 后，再把 `id` 写回 `manifest_snapshot`。
...
客户端                                服务端
  ...
  │  expo-runtime-version: 1.0.0        │ → app 解析成功后 INSERT manifest_requests
...
2. **管理 API**：管理员 JWT（完整管理权限）；§6.4 上传端点另接受 API Token（仅 plan / finalize，见 §6.5）
3. **首次部署**：env 注入 `INITIAL_ADMIN_USERNAME` / `INITIAL_ADMIN_PASSWORD`，服务首次启动时若 users 表为空则创建

API Token 的权限边界见 §6.5。它只覆盖 CI 所需的 plan / finalize；**publish 及**用户管理、签名 key 管理、回滚、删除、清理等操作必须使用管理员 JWT。
...
services:
  db:
    ...
    environment:
      ...
      POSTGRES_DB: ${POSTGRES_DB:-expo_ota}
      TZ: Asia/Shanghai
...
# —— PostgreSQL ——
POSTGRES_USER=admin
POSTGRES_PASSWORD=ChangeMeSecurely123!
POSTGRES_DB=expo_ota
...
# —— JWT (HS256, 32+ 字节随机字符串) ——
AUTH_ACCESS_SECRET=<random-secret-for-access-token>
AUTH_REFRESH_SECRET=<random-secret-for-refresh-token>
...
# 编辑填入真实值, 生成 AUTH_ACCESS_SECRET / AUTH_REFRESH_SECRET / SIGNING_KEY_ENCRYPTION_KEY:
openssl rand -hex 32      # access/refresh secret 各跑一次
openssl rand -base64 32   # SIGNING_KEY_ENCRYPTION_KEY
...
#   d) 跑 cli/publish.ts 验证（生成 pending draft）
#   e) 在 dashboard Updates 页点击 Publish 发布
```
...
| **M4 Manifest 端点**    | protocol-api 读 DB 组装 manifest, 处理 noUpdate / 404 / 406, 为已解析 App 写 manifest_requests | 2d   |
