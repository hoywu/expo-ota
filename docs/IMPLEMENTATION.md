...
- **多 keyid 轮换**：每 App 一个 code_signing_key
...
```text
                          ...
                          │  (sha256 寻址)   │  存储: COS apps/{slug}/assets/{sha256}
                          └──────────────────┘

独立实体:
  User        - 管理员账号 (无角色, 全等价)
  ApiToken    - CLI/CI 发布用, 关联 app_id
  CodeSignKey - 每 App 1 个, 私钥加密存 DB
...
- **Asset URL 不可变**：`manifest.assets[].url` 永远是 `https://<cos-domain>/apps/{appSlug}/assets/{sha256_base64url}`，永不改变
- **App 内 update 排序**：(app, runtimeVersion, platform) 流内按 `created_at DESC` 取最新一条为"当前"
...
└──────┬───────┘  └──────┬───────┘  └────────┬─────────┘
       ...
       └────────┬────────┘                   │
        ...
        └────────────────┘                   │
                                             │
外部依赖:
  - 腾讯云 COS (对象存储, 整桶 private, apps/*/assets/* 路径级公有读)
...
CREATE TABLE apps (
  ...
  app_slug                 text NOT NULL UNIQUE,    -- 创建时输入, 永久只读
  name                     text NOT NULL,
  description              text,
  code_signing_key_id      uuid,                    -- 软引用 code_signing_keys
...
-- =====================================================================
-- RuntimeVersion (懒创建: 第一次 update 自动插入; 也可预登记)
-- =====================================================================
...
CREATE TABLE updates (
  ...
  message               text,                      -- commit-like message
...
-- Stream 内 rank 查询 (用于删除保护: rnk > 3 可删)
CREATE INDEX updates_stream_idx
  ON updates (app_id, runtime_version_id, platform, created_at DESC)
  WHERE deleted_at IS NULL;
...
CREATE TABLE code_signing_keys (
  id                    uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  app_id                uuid NOT NULL REFERENCES apps(id) ON DELETE CASCADE,
  key_id                text NOT NULL,             -- 暴露给客户端
  algorithm             text NOT NULL DEFAULT 'rsa-v1_5-sha256',
  public_key_pem        text NOT NULL,
  encrypted_private_key bytea NOT NULL,            -- AES-256-GCM(随机 nonce || ciphertext)
  encryption_key_id     text NOT NULL,             -- env var 标识, 便于轮换
...
ALTER TABLE apps
  ADD CONSTRAINT apps_signing_key_fk
  FOREIGN KEY (code_signing_key_id) REFERENCES code_signing_keys(id);
...
CREATE TABLE manifest_requests (
  ...
  device_id         text,                          -- 来自 Expo-Extra-Params 或 X-Forwarded-For
  served_update_id  uuid,                          -- 服务端下发的 update id
...
CREATE TABLE client_events (
  ...
  -- 注意: 分区表 UNIQUE 必须含分区键 received_at, 严格 (app_id, event_id) 唯一改用:
  --   1) 应用层 INSERT ... WHERE NOT EXISTS (SELECT 1 ... WHERE app_id=$1 AND event_id=$2)
  --   2) 配 (app_id, event_id) 普通索引加速去重查
  -- 具体见 §5.6
...
CREATE TABLE audit_logs (
  ...
  action         text NOT NULL,                   -- publish_update / rollback_update / delete_update / cleanup_updates / create_app / create_user / ...
  target_type    text,
  target_id      text,
  request_id     text,
  ip             inet,
...
- **初始化新分区**（每月 1 号）：自动创建下月分区
- **清理过期分区**：`manifest_requests` 与 `client_events` 保留 6 个月（环境变量 `CLIENT_EVENTS_RETENTION_MONTHS` 控制）

实现在 `server/cmd/partition-gc/main.go`，通过系统 cron 或 go-zero 内置 cron 触发。
...
8. 构造 manifest JSON
9. 若 app.code_signing_key_id 非空:
...
```text
https://<cos-domain>/apps/{appSlug}/assets/{sha256_b64url}
```
...
- COS 路径：`apps/{slug}/assets/{sha256}.br` 与 `apps/{slug}/assets/{sha256}.gz`
...
2. 若 app.code_signing_key_id 非空 AND enabled:
...
**幂等**：PostgreSQL 分区表 UNIQUE 必须含分区键 `received_at`，严格 `(app_id, event_id)` 唯一无法在 schema 层做。改用应用层去重：

```sql
INSERT INTO client_events (...)
SELECT $1, $2, ...   -- 占位
WHERE NOT EXISTS (
  SELECT 1 FROM client_events WHERE app_id = $1 AND event_id = $2
);
```

普通索引 `client_events (app_id, event_id)` 加速去重查。
...
所有 `/api/admin/*` 端点需 `Authorization: Bearer <jwt-or-token>`，且属于以下三种之一：

- 管理员 JWT（从 `POST /api/admin/login` 获取，HS256，24h 过期）
- API Token（从 dashboard 创建，`token_hash` 存 DB）
...
| POST   | `/api/admin/apps/{appSlug}/signing-key:import`   | 导入公钥 + keyid（私钥留在客户端） |
  ...
  "missing": [
    {
      ...
      "finalUrl": "https://cos.example.com/apps/{slug}/assets/4nGjshg..."
    }
  ],
...
6. INSERT update（status='published'）
7. INSERT update_assets
8. 写 audit_log（`action='publish_update'`）
9. 失效相关缓存（Q5 暂未启用，POSTHOLE）
...
### 7.4 manifest_uuid 生成

`manifest_uuid` 在 publish 时**一次性计算并持久化**（不再每次请求重算）。算法：
...
async function main() {
  ...
  console.log(
    `Published update ${fin.updateId} (manifest ${fin.manifestUuid})`,
  );
}
...
1. 用 CTE 给每个 stream 内 update 算 rank
2. `rnk > keepLatestN` 的全部软删
3. 收集全部孤儿 asset，异步 GC

### 9.4 流定义

"流" = `(app_id, runtime_version, platform)`。每个流独立保留最新 N 条。
...
2. **管理 API**：JWT 或 API Token（Bearer）
...
`audit_logs` 表记录所有管理写操作（publish / rollback / delete / cleanup / create_app / create_user / change_password / ...），含 actor_user_id、IP、UA、payload、occurred_at。
...
# —— 客户端事件 retention (months) ——
CLIENT_EVENTS_RETENTION_MONTHS=6
MANIFEST_REQUESTS_RETENTION_MONTHS=6
```
...
# 2. 创建 COS 桶
#    - 整桶 private
#    - 在 COS 控制台配置路径 apps/*/assets/* 为公有读
...
| **M6 发布流**           | plan / COS pre-sign / 直传 / finalize 端到端 + cli/publish.ts                               | 2.5d |
| **M7 事件 + 可观测**    | events 端点 + dashboard 统计卡 + 分区 GC                                                    | 1.5d |
...
| **M10 生产加固**        | 限速中间件 / 安全响应头 / 健康检查 / nginx 限速 / cron GC                                   | 1d   |
